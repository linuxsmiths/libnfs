...
struct rpc_pdu *rpc_allocate_pdu2(struct rpc_context *rpc, int program, int version, int procedure, rpc_cb cb, void *private_data, zdrproc_t zdr_decode_fn, int zdr_decode_bufsize, size_t alloc_hint)
{
	...
	//printf("TOMAR: send_auth_tls=%d procedure=%d\n", send_auth_tls, procedure);
#endif /* HAVE_TLS */
...
#ifdef HAVE_TLS
	if (rpc->is_udp != 0 || pdu->expect_starttls) {
#else
	if (rpc->is_udp != 0) {
#endif
		unsigned int hash;
		...
		if (pdu->expect_starttls) {
			printf("TOMAR: Sending expect_starttls pdu inline (size=%d)\n",
				size);
		}
				...
				printf("TOMAR: iov[%d].iov_len=%ld\n",
						i, iov[i].iov_len);
                        }
                        //if (writev(rpc->fd, &iov[1], niov - 1) < 0) {
                        if (writev(rpc->fd, &iov[0], niov) < 0) {
                                rpc_set_error(rpc, "Sendto failed with errno %s", strerror(errno));
                                rpc_free_pdu(rpc, pdu);
                                return -1;
                        }
...
static int rpc_process_reply(struct rpc_context *rpc, ZDR *zdr)
{
	...
	case SUCCESS:
		 ...
		 * which case the server MUST support TLS else we must fail the
		 * RPC session.
		 */
		if (pdu->expect_starttls) {
			const char *starttls_str = "STARTTLS";
			const int starttls_len = 8;

			if (msg.body.rbody.reply.areply.verf.oa_flavor != AUTH_NONE) {
				RPC_LOG(rpc, 1,
					"Server sent bad verifier flavor (%d) in response to AUTH_TLS NULL RPC",
					msg.body.rbody.reply.areply.verf.oa_flavor);
				pdu->cb(rpc, RPC_STATUS_ERROR, "Server sent bad verifier flavor", pdu->private_data);
				break;
			} else if (msg.body.rbody.reply.areply.verf.oa_length != 8 ||
				   memcmp(msg.body.rbody.reply.areply.verf.oa_base, starttls_str, starttls_len)) {
				pdu->cb(rpc, RPC_STATUS_ERROR, "Server does not support TLS", pdu->private_data);
				break;
			}
		}
#endif
