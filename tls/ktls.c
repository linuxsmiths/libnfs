...
static bool tls_is_ktls_enabled(gnutls_session_t session, bool read)
{
	...
	if (ret == GNUTLS_E_UNIMPLEMENTED_FEATURE) {
		TLS_LOG(2, "gnutls version %s has kTLS support but kTLS is not enabled during build!",
...
static int ktls_setsockopt(int sock, bool read, const void *info, socklen_t infolen)
{
	...
	if (errno == EBUSY) {
		 ...
		 */
		TLS_LOG(1, "setsockopt(%s) returned(%d): %s", read ? "TLS_RX" : "TLS_TX", errno, strerror(errno));
		return 0;
	}

	TLS_LOG(1, "setsockopt(%s) failed(%d): %s", read ? "TLS_RX" : "TLS_TX", errno, strerror(errno));

	return -1;
}

#define CIPHER_TYPE(cipher)			TLS_CIPHER_##cipher
...
{ 									\
	...
	for (read = 0; read <= 1; read++) {				\
		...
		if (ret != GNUTLS_E_SUCCESS) { 				\
			TLS_LOG(1, "gnutls_record_get_state failed(%d)",\
				ret);					\
			return -1;					\
		}							\
									\
