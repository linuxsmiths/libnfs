...
int tls_global_init(struct rpc_context *rpc)
{
	...
	printf("TOMAR: tls_log_level set to %d\n", tls_log_level);
	...
	if (trusted_ca_file != NULL) {
		...
		if (ret < 0) {
			TLS_LOG(1, "tls_global_init: gnutls_certificate_set_x509_trust_file(%s) failed (%d): %s",
				trusted_ca_file, ret, gnutls_strerror(ret));
			/* Don't fail as yet, we fail if we are not able to load any certs from any sources */
		} else {
...
do_tls_handshake(struct rpc_context *rpc)
{
	...
	//if (rpc->tls_context.session == NULL)
		...
		TLS_LOG(2, "GNUTLS_NAME_DNS set to %s", server_sni);
		...
		TLS_LOG(2, "gnutls_session_set_verify_cert set to %s", server_sni);
	...
	printf("TOMAR: gnutls_handshake\n");
	ret = gnutls_handshake(session);
	printf("TOMAR: gnutls_handshake returned %d(%s)\n", ret, gnutls_strerror(ret));
	...
	if (ret < 0) {
...
#if 0
		{
			...
			int fd;
			certs = gnutls_certificate_get_peers(session, &list_size);
			printf("TOMAR: list_size=%d\n", list_size);
			TLS_LOG(2, "certs[0].size = %d", certs[0].size);
			fd = open("/tmp/cert.der", O_RDWR|O_CREAT);
			printf("fd = %d\n", fd);
			ret = write(fd, certs[0].data, certs[0].size);
			printf("ret = %d\n", ret);
			close(fd);
		}
#endif
